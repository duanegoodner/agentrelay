...
build_cross_package_arrows = generate_overview_mod.build_cross_package_arrows
CrossPackageArrow = generate_overview_mod.CrossPackageArrow
...
class TestGenerateOverviewD2:
    ...
    def test_contains_link(self) -> None:
        ...
        arrows = build_overview_arrows(rels, pkg_ids)
        output = generate_overview_d2(packages, arrows)
        assert 'link: "diagram-detailed.svg"' in output


# ---------------------------------------------------------------------------
# End-to-end with real diagram-detailed.d2
# ---------------------------------------------------------------------------

_REAL_DIAGRAM = Path(__file__).resolve().parents[2] / "docs" / "diagram-detailed.d2"


@pytest.mark.skipif(not _REAL_DIAGRAM.exists(), reason="diagram-detailed.d2 not found")
class TestEndToEnd:
    def test_package_count(self) -> None:
        lines = _REAL_DIAGRAM.read_text().splitlines()
        packages = parse_packages(lines)
        # Current diagram has 12 top-level containers
...
# ---------------------------------------------------------------------------
# build_cross_package_arrows
# ---------------------------------------------------------------------------


class TestBuildCrossPackageArrows:
    def test_excludes_intra_package(self) -> None:
        rels = parse_relationships(SAMPLE_D2.splitlines())
        pkg_ids = {"pkg_a", "pkg_b", "pkg_c"}
        arrows = build_cross_package_arrows(rels, pkg_ids)
...
class TestGenerateOverviewHtml:
    def test_inlines_svg(self) -> None:
        html = generate_overview_html(_REAL_OVERVIEW_SVG, "diagram-detailed.svg")
        assert "<svg" in html
        assert "</svg>" in html

    def test_contains_popup_js(self) -> None:
        html = generate_overview_html(_REAL_OVERVIEW_SVG, "diagram-detailed.svg")
        assert "addEventListener" in html
        assert "ARROW_CONNECTIONS" in html

    def test_contains_detail_link(self) -> None:
        html = generate_overview_html(_REAL_OVERVIEW_SVG, "diagram-detailed.svg")
        assert 'href="diagram-detailed.svg"' in html
    ...
    def test_no_package_click_handler(self) -> None:
        """Package nodes should not be interactive."""
        html = generate_overview_html(_REAL_OVERVIEW_SVG, "diagram-detailed.svg")
        # Should not contain package tooltip or SVG popup code
        assert "PKG_TOOLTIPS" not in html
        assert "PKG_SVGS_B64" not in html

    def test_arrow_connections_with_arrows(self) -> None:
        arrows = [
            CrossPackageArrow(
                source_pkg="pkg_a",
                target_pkg="pkg_b",
        ...
        html = generate_overview_html(
            _REAL_OVERVIEW_SVG, "diagram-detailed.svg", arrows=arrows
        )
        assert "pkg_a|pkg_b" in html
        assert "ClassA" in html


# ---------------------------------------------------------------------------
# End-to-end cross-package arrows with real diagram
# ---------------------------------------------------------------------------


@pytest.mark.skipif(not _REAL_DIAGRAM.exists(), reason="diagram-detailed.d2 not found")
class TestCrossPackageArrowsEndToEnd:
    def test_arrow_count_range(self) -> None:
        lines = _REAL_DIAGRAM.read_text().splitlines()
        packages = parse_packages(lines)
        pkg_ids = {p.id for p in packages}
    ...
    def test_all_arrows_have_details(self) -> None:
        lines = _REAL_DIAGRAM.read_text().splitlines()
        packages = parse_packages(lines)
        pkg_ids = {p.id for p in packages}
        rels = parse_relationships(lines)
        arrows = build_cross_package_arrows(rels, pkg_ids)
        for arrow in arrows:
            assert len(arrow.details) > 0

    def test_no_intra_package(self) -> None:
        lines = _REAL_DIAGRAM.read_text().splitlines()
        packages = parse_packages(lines)
        pkg_ids = {p.id for p in packages}
        rels = parse_relationships(lines)
