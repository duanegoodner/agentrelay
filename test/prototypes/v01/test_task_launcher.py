...
def test_create_worktree_calls_git(tmp_path):
    ...
    with patch("agentrelay.prototypes.v01.task_launcher.subprocess.run") as mock_run:
...
def test_create_worktree_sets_state(tmp_path):
    task = make_task()
    target_repo = tmp_path / "target"
    with patch("agentrelay.prototypes.v01.task_launcher.subprocess.run"):
...
def test_launch_agent_creates_tmux_window(tmp_path):
    ...
    with (
        patch(
            "agentrelay.prototypes.v01.task_launcher.subprocess.check_output",
            return_value=b"%3\n",
        ) as mock_out,
        patch("agentrelay.prototypes.v01.task_launcher.subprocess.run"),
    ):
...
def test_launch_agent_sends_claude_command(tmp_path):
    ...
    with (
        ...
        patch("agentrelay.prototypes.v01.task_launcher.subprocess.run") as mock_run,
    ):
        launch_agent(task, "agentrelay")
    cmd = mock_run.call_args[0][0]
...
def test_send_prompt_accepts_trust_dialog():
    with (
        patch("agentrelay.prototypes.v01.task_launcher.time.sleep"),
        patch("agentrelay.prototypes.v01.task_launcher.subprocess.run") as mock_run,
        patch(
            "agentrelay.prototypes.v01.task_launcher._wait_for_claude_tui",
            return_value=True,
        ),
    ):
        send_prompt(
...
def test_send_prompt_sleeps_twice():
    with (
        patch("agentrelay.prototypes.v01.task_launcher.time.sleep") as mock_sleep,
        patch("agentrelay.prototypes.v01.task_launcher.subprocess.run"),
        patch(
            "agentrelay.prototypes.v01.task_launcher._wait_for_claude_tui",
            return_value=True,
...
def test_wait_for_claude_tui_returns_true_when_bypass_present():
    from agentrelay.prototypes.v01.task_launcher import _wait_for_claude_tui
...
def test_wait_for_claude_tui_returns_false_on_timeout():
    ...
    with (
        ...
        patch("agentrelay.prototypes.v01.task_launcher.time.sleep"),
        patch(
            "agentrelay.prototypes.v01.task_launcher.time.time",
...
def test_merge_pr_reattempts_on_not_mergeable():
    ...
    with (
        patch(
            "agentrelay.prototypes.v01.task_launcher.subprocess.run",
            side_effect=results,
        ),
        patch("agentrelay.prototypes.v01.task_launcher.time.sleep") as mock_sleep,
    ):
...
def test_neutralize_calls_git_fetch_and_checkout(tmp_path):
    ...
    with patch(
        "agentrelay.prototypes.v01.task_launcher.subprocess.run", side_effect=fake_run
    ) as mock_run:
